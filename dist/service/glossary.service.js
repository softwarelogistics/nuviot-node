"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GlossaryService = void 0;
var core_1 = require("@angular/core");
var GlossaryService = /** @class */ (function () {
    function GlossaryService(nuviotClient) {
        this.nuviotClient = nuviotClient;
    }
    GlossaryService.prototype.getGlossaries = function () {
        return this.nuviotClient.request('/api/glossaries');
    };
    GlossaryService.prototype.createGlossary = function () {
        return this.nuviotClient.request('/api/glossary/factory');
    };
    GlossaryService.prototype.getGlossary = function (id) {
        return this.nuviotClient.request("/api/glossary/".concat(id));
    };
    GlossaryService.prototype.addGlossary = function (glossary) {
        return this.nuviotClient.post('/api/glossary', glossary);
    };
    GlossaryService.prototype.addNewGlossary = function (glossary) {
        return this.nuviotClient.post('/api/glossary', glossary);
    };
    GlossaryService.prototype.updateGlossary = function (glossary) {
        return this.nuviotClient.update('/api/glossary', glossary);
    };
    GlossaryService = __decorate([
        (0, core_1.Injectable)({
            providedIn: 'root'
        })
    ], GlossaryService);
    return GlossaryService;
}());
exports.GlossaryService = GlossaryService;
